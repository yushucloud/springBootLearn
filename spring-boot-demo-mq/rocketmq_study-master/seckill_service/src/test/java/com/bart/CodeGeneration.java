...
public class CodeGeneration {
    public static void main(String[] args) {
        ...
        DataSourceConfig dsc = new DataSourceConfig.Builder(
                "jdbc:mysql://127.0.0.1:3306/dljd_seckill?useUnicode=true&characterEncoding=UTF-8&serverTimezone=UTC", "root", "123456")
...
//        String projectPath = System.getProperty("user.dir"); //获取项目路径
        String projectPath = "D://entity//"; //获取项目路径
...
//                .fileOverride() // 覆盖生成文件 3.5.3 后过期，已迁移到策略配置中 com.baomidou.mybatisplus.generator.config.StrategyConfig
                .build();
        ...
        PackageConfig packages = new PackageConfig.Builder()
                .parent("com.bart")//父包名。如果为空，将下面子包名必须写全部， 否则就只需写子包名
                .entity("api.model")//实体类包名
                .controller("provider.controller")//控制层包名
                .mapper("provider.dao")//mapper层包名
                .xml("provider.dao.mapper")//数据访问层xml包名
                .service("provider.service")//service层包名
                .serviceImpl("provider.service.impl")//service实现类包名
//                .other("output")//输出自定义文件时的包名
//                .pathInfo(Collections.singletonMap(OutputFile.mapper, projectPath + "/src/main/resources/mapper")) //路径配置信息,就是配置各个文件模板的路径信息,这里以mapper.xml为例
                .build();
        /**
         * 模板配置
         */
...
//            .build();
        ...
        StrategyConfig strategyConfig = new StrategyConfig.Builder()
                ...
                //.likeTable()模糊表匹配
                .addInclude("goods", "order_records")//添加表匹配，指定要生成的数据表名，不写默认选定数据库所有表
